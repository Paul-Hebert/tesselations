...
function generateCommandsForSide(length, variance, mainAxis, secondAxis) {
  ...
  commands.forEach(function (command, index, array) {
    ...
    if (index === array.length - 1) {
      ...
      if (change + secondAxisDrift > variance) {
        variance * -1;
      }
    ...
    if (command.type === "Q") {
      ...
      command.control[secondAxis] = Math.round(
        command[secondAxis] + command[secondAxis] * Math.random()
      );
    }
  });
...
function addSide({
  ...
  return commands.map((command) => {
    ...
    if (command.type === "Q") {
      ...
      controlPoint[secondAxis] =
        startPoint[secondAxis] + command.control[secondAxis];
    }
